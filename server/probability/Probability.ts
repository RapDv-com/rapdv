...
export class Probability {
  ...
  public static getRandomSubarray<T>(size: number, source: Array<T>): Array<T> {
    if (!source || source.length === 0) return [];
    size = Math.min(size, source.length);
    const result = [];
    for (let i = 0; i < size; i++) {
      let element = undefined;
      const maxTries = source.length;
      let tryCount = 0;

      while (element === undefined || result.indexOf(element) >= 0) {
        element = Probability.getRandomElement(source);
        tryCount++;
